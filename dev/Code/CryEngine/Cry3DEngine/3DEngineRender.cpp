...
ASYNC_RENDER_ENABLED_END
    ...
    if (m_pMergedMeshesManager != nullptr)
    {
    ...
    *szLevelName = 0;
    ...
    if (bShowAnimationOutOfBudget && gEnv->pCharacterManager)
    {
        ...
        if (0 == (GetRenderer()->GetFrameID(false) & 15))
        {
            if (bOutOfMem)
            {
                sprintf_s(szAnimString, "Animation Keys:  %.1f", animMem.m_nAnimsCurrent / (float)(1024 * 1024));
            }
        }

        if (bOutOfMem)
        {
            DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY, DISPLAY_INFO_SCALE, bOutOfMem ? Col_Red : Col_White, szAnimString);
        }
    }
...
# define MAX_PLE_TIME 4.f
        ...
        IF (pPartMan != NULL, 1)
        {
            ...
            partTicks = 1000.0f * gEnv->pTimer->TicksToSeconds(pPartMan->NumFrameTicks());
            float fTimeSyncMS = 1000.0f * gEnv->pTimer->TicksToSeconds(pPartMan->NumFrameSyncTicks());

            DrawTextRightAligned(fTextPosX, fTextPosY += (fTextStepY - STEP_SMALL_DIFF),
                DISPLAY_INFO_SCALE_SMALL, partTicks > maxVal ? Col_Red : Col_White, "%.2f(%.2f) ms(%3d)      Part", partTicks, fTimeSyncMS, nNumEmitter);
        }
...
#undef CONVX
    ...
    if (GetCVars()->e_ParticlesDebug & 1)
    {
        ...
        DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
            "Particle %5.0f/%5.0f/%5.0f, Emitter %3.0f/%3.0f/%3.0f, SubEmitter: %3.0f, Fill %5.2f/%5.2f",
        ...
        {
            DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
                "Particle: Reiter %4.0f, Reject %4.0f, Clip %4.1f, Coll %4.1f / %4.1f",
...
#define DRAW_OBJ_STATS(_var) DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY, "%s: %d", (#_var), GetInstCount(_var))
    ...
    if (GetCVars()->e_TerrainTextureStreamingDebug && legacyTerrain)
    {
        ...
        if (legacyTerrain->TryGetTextureStatistics(statistics))
        {
            DrawTextRightAligned(
                fTextPosX,
                fTextPosY += fTextStepY,
                "Terrain texture streaming status: resident=%d, streaming=%d, waiting=%d, all=%d, pool=%d",
                statistics.residentCount,
                statistics.streamingCount,
                statistics.waitingCount,
                statistics.activeTotalCount,
                statistics.poolCapacity);
        }
    }
    ...
    if (GetCVars()->e_MergedMeshesDebug)
    {
        ...
        DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
            "%d active nodes, number of instances : %d (visible %d)",
    ...
    if (IParticleManager* pPartMan = gEnv->p3DEngine->GetParticleManager())
    {
        pPartMan->ResetFrameTicks();
    }

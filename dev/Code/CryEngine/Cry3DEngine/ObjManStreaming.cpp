...
static bool AreTexturesStreamed(_smart_ptr<IMaterial> pMaterial, float fMipFactor)
{
...
void CObjManager::PrecacheCharacterCollect(IRenderNode* pObj, const float fImportance, ICharacterInstance* pCharacter, _smart_ptr<IMaterial> pSlotMat, const Matrix34& matParent,
    ...
    const int nRoundId, std::vector<std::pair<_smart_ptr<IMaterial>, float> >& collectedMaterials)
{
        ...
        if (IAttachment* pAtt = pAttMan->GetInterfaceByIndex(i))
        {
            ...
            if (pIAttachmentObject)
            {
                ...
                if (pIAttachmentSkin)
                {
                    ...
                    _smart_ptr<IMaterial> pAttMatOverride = (_smart_ptr<IMaterial>)pIAttachmentObject->GetReplacementMaterial();
                    _smart_ptr<IMaterial> pAttMat = pAttMatOverride ? pAttMatOverride : (_smart_ptr<IMaterial>)pIAttachmentObject->GetBaseMaterial();
                    ...
                    for (int currentLod = minPrecacheLod; currentLod <= maxPrecacheLod; ++currentLod)
                    {
                        ...
                        _smart_ptr<IMaterial> pCharObjMat = pISkin->GetIMaterial(currentLod);
                ...
                if (pStatObj)
                {
                    ...
                    for (int currentLod = minPrecacheLod; currentLod <= maxPrecacheLod; ++currentLod)
                    {
                        ...
                        _smart_ptr<IMaterial> pAttMatOverride = (_smart_ptr<IMaterial>)pIAttachmentObject->GetReplacementMaterial();
                        _smart_ptr<IMaterial> pAttMat = pAttMatOverride ? pAttMatOverride : (_smart_ptr<IMaterial>)pIAttachmentObject->GetBaseMaterial();
                        _smart_ptr<IMaterial> pMaterial = pAttMat ? pAttMat : pStatObj->GetMaterial();
    ...
    _smart_ptr<IMaterial> pCharObjMat = pCharacter->GetIMaterial();
...
void CObjManager::PrecacheCharacter(IRenderNode* pObj, const float fImportance, ICharacterInstance* pCharacter, _smart_ptr<IMaterial> pSlotMat, const Matrix34& matParent,
    const float fEntDistance, const float fScale, int nMaxDepth, bool bFullUpdate, bool bDrawNear, int nLod)
{
    ...
    if (numMaterials > 0)
    {
        ...
        {
            const std::pair<_smart_ptr<IMaterial>, float>& collectedMaterial = m_collectedMaterials[i];
            CMatInfo* pMaterial = ((CMatInfo*)collectedMaterial.first.get());

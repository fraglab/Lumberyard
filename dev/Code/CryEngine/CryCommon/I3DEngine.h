...
{    
  ...
  // Returns 'false' on error.
    ...
    virtual void PrecacheCharacter(IRenderNode* pObj, const float fImportance,  ICharacterInstance* pCharacter, _smart_ptr<IMaterial> pSlotMat, const Matrix34& matParent, const float fEntDistance, const float fScale, int nMaxDepth, bool bForceStreamingSystemUpdate, const SRenderingPassInfo& passInfo) = 0;
...
inline void SRenderingPassInfo::InitRenderingFlags(uint32 nRenderingFlags)
{
    ...
    if (pBrushes->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::BRUSHES;
    }
    if (pVegetation->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::VEGETATION;
    }
    if (pEntities->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::ENTITIES;
    }
    if (pTerrain->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::TERRAIN;
    }
    if (pWaterOcean->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::WATEROCEAN;
    }
    if (pParticles->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::PARTICLES;
    }
    ...
    if (pRoads->GetIVal() == 0)
    {
        m_nRenderingFlags &= ~SRenderingPassInfo::ROADS;
    }
